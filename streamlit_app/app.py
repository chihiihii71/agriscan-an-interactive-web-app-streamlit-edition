...
CHART_HEIGHT = 360  # single source of truth — image and chart share this height
...
def resize_to_height(image: Image.Image, target_height: int) -> Image.Image:
    """Resize image so its height equals target_height, preserving aspect ratio."""
    w, h = image.size
    if h == target_height:
        return image
...
if uploaded_file is not None:
    ...
    st.markdown(f"""
        <div style="background-color:#1e4620;padding:20px;border-radius:10px;
            ...
            <h4 style="color:#a7f3d0;margin:5px 0 0 0">
                {T["confidence"]}: {pred_prob:.2f}%
            </h4>
        </div>
    """, unsafe_allow_html=True)

    # --- 2. Side-by-Side Image & Chart — same height ---
    col_img, col_chart = st.columns([1, 1.2])

    with col_img:
        # Resize the image so its height matches CHART_HEIGHT exactly.
        # This keeps both columns the same visual height so the report
        # section appears immediately below without scrolling.
        display_img = resize_to_height(image, CHART_HEIGHT)
        st.image(display_img, caption=label)
    ...
    # --- 3. Report — appears immediately below both columns ---
    with st.expander(T["prob_table"], expanded=False):
        col_table, col_btn = st.columns([3, 1])

        with col_table:
            st.dataframe(
                df_sorted.style.format({"Probability": "{:.2f}%"}),
